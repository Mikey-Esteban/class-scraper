...
def run():
    ...
    new_classes = [['Studio','Start time', 'Classes', 'Link', 'Instructor']]
    for thing in classes_for_date:
        ...
        teacher = split[1]
        new_classes.append(['Gramercy Studios', class_time, class_name, 'unavailable', teacher])
    ...
    if len(new_classes) > 1:
        pd.DataFrame(new_classes).to_csv(f"csv/{get_date()}/live/gramercy.csv", header=False, index=False)
